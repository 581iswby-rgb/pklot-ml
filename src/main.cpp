...
std::array<Scores, kHorizons.size()> train_model(const fs::path& input, Model& model) {
    ...
    const std::int64_t split_time = samples[samples.size() * 8 / 10].timestamp_epoch;
    std::vector<Sample> training;
    std::vector<Sample> testing;
    for (const auto& sample : samples) {
        (sample.timestamp_epoch < split_time ? training : testing).push_back(sample);
    }
    if (training.empty() || testing.empty()) {
        throw std::runtime_error("not enough data for temporal train/test split");
    }
    ...
    for (std::size_t horizon = 0; horizon < kHorizons.size(); ++horizon) {
        ...
        for (const auto& sample : testing) {
            ...
            const double model_error = forecast(model, horizon, sample.features) - target;
        ...
        const double count = static_cast<double>(testing.size());
        scores[horizon] = {model_absolute / count, std::sqrt(model_squared / count),
...
Model load_model(const fs::path& input) {
    ...
    if (!(file >> signature >> feature_count) || signature != "PKLOT_ML_V1" ||
...
void write_metrics(const fs::path& output,
                   const std::array<Scores, kHorizons.size()>& scores) {
    ensure_parent(output);
    std::ofstream file(output);
    ...
    for (std::size_t index = 0; index < scores.size(); ++index) {
        ...
        file << "  \"" << kHorizons[index] << "\": {\"model_mae\": " << score.model_mae
...
void train_command(const fs::path& input, const fs::path& model_path,
    ...
    write_metrics(metrics_path, scores);
}
